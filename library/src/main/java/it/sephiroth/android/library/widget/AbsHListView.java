...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements ViewTreeObserver.OnGlobalLayoutListener,
	 ...
	 * 表示我们刚刚收到的触摸事件，我们都在观望，如果它是一个龙头或滚动手势。
	 */
	...
	public void setItemChecked( int position, boolean value ) {
		...
		if ( Build.VERSION.SDK_INT >= 11 ) {
			if ( value && mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL && mChoiceActionMode == null ) {
				...
				if(mMultiChoiceModeCallback instanceof MultiChoiceModeWrapper){
					mChoiceActionMode = startActionMode( (MultiChoiceModeWrapper)mMultiChoiceModeCallback );
				}
			}
		}

		if ( mChoiceMode == ListView.CHOICE_MODE_MULTIPLE || ( Build.VERSION.SDK_INT >= 11 && mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL ) ) {
	...
	protected void invokeOnItemScrollListener() {
		if ( mOnScrollListener != null ) {
			XqLog.d("invokeOnItemScrollListener.onScroll");
	 ...
	 * created.
	 * 一个基类的Runnable，将检查他们的观点仍连接到原始窗口Runnable对象被创建的时候。
	 */
			...
			if ( mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL ) {
				if(mMultiChoiceModeCallback instanceof MultiChoiceModeWrapper){
					if ( mChoiceActionMode == null && ( mChoiceActionMode = startActionMode( (MultiChoiceModeWrapper)mMultiChoiceModeCallback ) ) != null ) {
						...
						/**
						 * 该框架将提供一些内置的行为，比如长按触觉反馈，但您可能希望提供反馈，自己的小部件。
						 */
				...
				if ( child != null && !child.hasFocusable() ) {
					...
					if ( !mDataChanged ) {
						...
						if ( longClickable ) {
							...
							XqLog.d("UUUUU","postDelayed( mPendingCheckForLongPress, longPressTimeout )");
	...
	private boolean startScrollIfNeeded( int x ) {
		...
		if ( overscroll || distance > mTouchSlop ) {
			...
			XqLog.d("startScrollIfNeeded.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL");
		...
		if ( mTouchMode == TOUCH_MODE_SCROLL ) {
			...
			if ( x != mLastX ) {
				...
				if ( incrementalDeltaX != 0 ) {
					XqLog.d("scrollIfNeeded.mTouchMode == TOUCH_MODE_SCROLL.trackMotionScroll");
			...
			if ( x != mLastX ) {
				...
				if ( incrementalDeltaX != 0 ) {
					...
					XqLog.d("scrollIfNeeded.mTouchMode == TOUCH_MODE_OVERSCROLL.trackMotionScroll");
	...
	public boolean onTouchEvent( MotionEvent ev ) {
		...
		switch ( action & MotionEvent.ACTION_MASK ) {
			case MotionEvent.ACTION_DOWN: {
				XqLog.d("MotionEvent.ACTION_DOWN");
				switch ( mTouchMode ) {
					...
					default: {
						...
						if ( !mDataChanged ) {
							if ( ( mTouchMode != TOUCH_MODE_FLING ) && ( motionPosition >= 0 )
								...
								// proven otherwise
								//用户点击了一个实际的视图（并没有停止一扔）。
								//这可能是一个点击或滚动。假设它是一个点击，直到证明并非如此
								mTouchMode = TOUCH_MODE_DOWN;
								// FIXME Debounce
								if ( mPendingCheckForTap == null ) {
									mPendingCheckForTap = new CheckForTap();
								}
								XqLog.d("UUUUU","postDelayed(mPendingCheckForTap, ViewConfiguration.getTapTimeout())");
			...
			case MotionEvent.ACTION_MOVE: {
				XqLog.d("MotionEvent.ACTION_MOVE");
			...
			case MotionEvent.ACTION_UP: {
				XqLog.d("MotionEvent.ACTION_UP");
				switch ( mTouchMode ) {
					case TOUCH_MODE_DOWN:
					case TOUCH_MODE_TAP:
					case TOUCH_MODE_DONE_WAITING:
						XqLog.d("TOUCH_MODE_DOWN|TOUCH_MODE_TAP|TOUCH_MODE_DONE_WAITING");
						final int motionPosition = mMotionPosition;
						final View child = getChildAt( motionPosition - mFirstPosition );
					...
					case TOUCH_MODE_SCROLL:
						...
						if ( childCount > 0 ) {
							...
							if ( mFirstPosition == 0 && firstChildLeft >= contentLeft && mFirstPosition + childCount < mItemCount
								...
								mTouchMode = TOUCH_MODE_REST;
								XqLog.d("mTouchMode = TOUCH_MODE_REST");
								...
								if ( Math.abs( initialVelocity ) > mMinimumVelocity
									...
									XqLog.d("mTouchMode = SCROLL_STATE_FLING");
									reportScrollStateChange(OnScrollListener.SCROLL_STATE_FLING);
									mFlingRunnable.start( -initialVelocity );
								} else {
									mTouchMode = TOUCH_MODE_REST;
									XqLog.d("mTouchMode = SCROLL_STATE_IDLE");
									reportScrollStateChange( OnScrollListener.SCROLL_STATE_IDLE );
									if ( mFlingRunnable != null ) {
										XqLog.d("mFlingRunnable.endFling()");
										mFlingRunnable.endFling();
									}else {
										resetInCenter();
									}
									if ( mPositionScroller != null ) {
										XqLog.d("mPositionScroller.stop()");
										mPositionScroller.stop();
									}
								}
							}
			...
			case MotionEvent.ACTION_CANCEL: {
				XqLog.d("MotionEvent.ACTION_CANCEL");
				switch ( mTouchMode ) {
					case TOUCH_MODE_OVERSCROLL:
						if ( mFlingRunnable == null ) {
							mFlingRunnable = new FlingRunnable();
			...
			case MotionEvent.ACTION_POINTER_UP: {
				XqLog.d("MotionEvent.ACTION_POINTER_UP");
			...
			case MotionEvent.ACTION_POINTER_DOWN: {
				XqLog.d("MotionEvent.ACTION_POINTER_DOWN");
	...
	public void draw( Canvas canvas ) {
		...
		if ( mEdgeGlowLeft != null ) {
			...
			if ( !mEdgeGlowLeft.isFinished() ) {
				...
				// mEdgeGlowLeft.setSize(height, height);
				...
				if ( mEdgeGlowLeft.draw( canvas ) ) {
					//mEdgeGlowLeft.setPosition( edgeX, topPadding );
					invalidate();
				}
			...
			if ( !mEdgeGlowRight.isFinished() ) {
				...
				mEdgeGlowRight.setSize( height, height );

				if ( mEdgeGlowRight.draw( canvas ) ) {
					// Account for the rotation
					// mEdgeGlowRight.setPosition( edgeX + width, edgeY );
					invalidate();
				}
				canvas.restoreToCount( restoreCount );
			}
	...
	private class FlingRunnable implements Runnable {
		...
		private final Runnable mCheckFlywheel = new Runnable() {
			...
			public void run() {
				...
				if ( Math.abs( xvel ) >= mMinimumVelocity && scroller.isScrollingInDirection( xvel, 0 ) ) {
					...
					endFling();
					mTouchMode = TOUCH_MODE_SCROLL;
					XqLog.d("FlingRunnable.mTouchMode = TOUCH_MODE_SCROLL");
					reportScrollStateChange(OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
				}
			}
		};
		...
		void startSpringback() {
			if ( mScroller.springBack( getScrollX(), 0, 0, 0, 0, 0 ) ) {
				...
				mTouchMode = TOUCH_MODE_REST;
				XqLog.d("FlingRunnable.startSpringback.OnScrollListener.SCROLL_STATE_IDLE");
				reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE );
			}
		}
...
//			XqLog.d("FlingRunnable.endFling.OnScrollListener.SCROLL_STATE_IDLE");
			reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
			clearScrollingCache();
	...
	public void smoothScrollBy( int distance, int duration, boolean linear ) {
		...
		if ( distance == 0 || mItemCount == 0 || childCount == 0 ||
			...
			XqLog.d("smoothScrollBy.OnScrollListener.SCROLL_STATE_FLING");
			reportScrollStateChange(OnScrollListener.SCROLL_STATE_FLING );
	...
	boolean trackMotionScroll( int deltaX, int incrementalDeltaX ) {
		...
		XqLog.d("trackMotionScroll.invokeOnItemScrollListener");
		invokeOnItemScrollListener();
	...
	boolean resurrectSelection() {
		...
		if ( selectedPos >= firstPosition && selectedPos <= getLastVisiblePosition() ) {
			...
			XqLog.d("resurrectSelection.invokeOnItemScrollListener");
			invokeOnItemScrollListener();
		} else {
			selectedPos = INVALID_POSITION;
		}
		XqLog.d("resurrectSelection.OnScrollListener.SCROLL_STATE_IDLE");
		reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE );
